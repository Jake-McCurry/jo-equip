...
OUTPUT = ROOT / ".agents/outputs/adventure-visual-proof-v3.pdf"
...
def recompose_page(output, source, source_pno, clips_and_shifts, image_path=None, img_rect=None, labels_func=None):
    ...
    for clip_rect, shift_y in clips_and_shifts:
        ...
        # Redact out everything outside the clip_rect so it's not present in the new page
...
def main():
    ...
    for source_page_number in range(1, source.page_count + 1):
        if source_page_number in {12, 16, 17, 18, 26, 30, 35}:
            # Custom handling creates replacement pages
            if source_page_number == 12:
                recompose_page(
                    output, source, 12, 
                    clips_and_shifts=[
                        ((0, 0, 612, 190), 0),
                    ...
                    img_rect=(63, 210, 549, 560)
                )

            elif source_page_number == 16:
                def add_labels_16(p):
                    label(p, (105, 370, 300, 395), "Self-Directed", 11)
                    label(p, (312, 370, 507, 395), "Christ-Directed", 11)
                recompose_page(
                    output, source, 16, 
                    clips_and_shifts=[
                        ((0, 0, 612, 215), 0),
                    ...
                    img_rect=(90, 225, 522, 365),
                    labels_func=add_labels_16
                )

            elif source_page_number == 17:
                def add_labels_17(p):
                    label(p, (99, 730, 231, 770), "Yield yourself\nto God", 9)
                    label(p, (240, 730, 372, 770), "Confess your\nsins", 9)
                    label(p, (381, 730, 513, 770), "Trust God to be\nin control", 9)
                recompose_page(
                    output, source, 17, 
                    clips_and_shifts=[
                        ((0, 0, 612, 540), 0)
                ...
                new_infographic_page = output.new_page(width=612, height=792)
                title_bar(new_infographic_page, "Five Ways to Receive God's Word")
                insert_contain(new_infographic_page, (63, 135, 549, 580), prepared["word"])
                
                temp_doc = fitz.open()
                temp_doc.insert_pdf(source, from_page=25, to_page=25)
                tpage = temp_doc[0]
                tpage.add_redact_annot(fitz.Rect(0, 0, 612, 620))
                tpage.apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE)
                
                src_rect = fitz.Rect(0, 620, 612, 792)
                shift_y = -20
                dest_rect = src_rect + (0, shift_y, 0, shift_y)
                new_infographic_page.show_pdf_page(dest_rect, temp_doc, 0, clip=src_rect)
                
                for link in source[25].links():
                    cx = (link["from"].x0 + link["from"].x1)/2
                    cy = (link["from"].y0 + link["from"].y1)/2
                    if src_rect.contains(fitz.Point(cx, cy)):
                        new_link = link.copy()
                        new_link["from"] = link["from"] + (0, shift_y, 0, shift_y)
                        new_infographic_page.insert_link(new_link)
            ...
            if source_page_number == 5:
                ...
                # Add Faith photo immediately AFTER page 7
