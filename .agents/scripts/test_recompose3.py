...
def recompose_page(output, source, source_pno, clips_and_shifts, image_path=None, img_rect=None, labels_func=None):
    ...
    for clip_rect, shift_y in clips_and_shifts:
        ...
        # Create a temp doc to hold the redacted page
        temp_doc = fitz.open()
        temp_doc.insert_pdf(source, from_page=source_pno-1, to_page=source_pno-1)
        tpage = temp_doc[0]
        
        # Redact everything outside src_rect
        if src_rect.y0 > 0:
...
test_pdf = ROOT / ".agents/outputs/adventure-layout-review/test_recompose3.pdf"
out.save(test_pdf)
subprocess.run(["pdftotext", "-layout", str(test_pdf), str(ROOT / ".agents/outputs/adventure-layout-review/test_recompose3.txt")])
with open(ROOT / ".agents/outputs/adventure-layout-review/test_recompose3.txt") as f:
    print(f.read())
