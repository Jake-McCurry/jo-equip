...
Live Mailchimp signup via shared component `src/components/MakeMultiplySignupForm.astro` (direct POST to `jesusonline.us1.list-manage.com`, honeypot bot field, audience tag 5798198; fields EMAIL/FNAME/CHURCH/COUNTRY/MMERGE8-Gender). The SAME form is used on `/categories/church/make-multiply-disciples-newsletter` (via `MakeMultiplyNewsletter.astro`) — keep them in sync by editing only the shared component. `/newsletter` accent blue `#0083de`; channel page accent plum. Former Virtuous embed removed July 2026. Linked from hero CTA, `/more`, footer.
