...
export function ConcordancePrototype() {
  ...
  return <div className="flex flex-col min-h-[100dvh] text-[var(--color-text,#003A66)]" style={{ background: "var(--color-surface, #FFFDFB)", fontFamily: "var(--font-sans, 'Source Sans 3', system-ui, sans-serif)" }}>
    ...
    <div className="kg-warm-theme flex-1 flex flex-col bg-[var(--color-surface,#FFFDFB)]">
      ...
      <section className="kg-reading min-w-0 max-w-[920px] px-5 py-8 md:px-10 md:py-12 lg:px-14">{!selectedIndex ? <div role="status" className="kg-sans py-16 text-center text-base text-[var(--color-text-muted,#2E5A7A)]">Loading selected topic…</div> : !selected ? <div className="kg-sans py-16 text-center text-base text-[var(--color-text-muted,#2E5A7A)]">{payloadError[selectedIndex.letter] ? <div role="alert" className="text-[var(--color-action-warm,#C45100)]">{payloadError[selectedIndex.letter]} <button className="underline" onClick={() => loadLetter(selectedIndex.letter).catch(() => undefined)}>Try again</button></div> : <div role="status">Loading {selectedIndex.letter} topics…</div>}</div> : <><div className="kg-no-print mb-6 flex items-center gap-2 kg-sans text-sm text-[var(--color-text-muted,#2E5A7A)]"><BookOpen size={16}/>Topic study <ChevronRight size={14}/>{selected.title}</div><div className="mb-8 flex flex-col gap-4 border-b border-[var(--color-border-soft,#CCEBFF)] pb-7 sm:flex-row sm:justify-between"><div><p className="kg-sans text-xs font-bold uppercase tracking-[.22em] text-[var(--color-action-warm,#C45100)]">{selected.recordType === "cross-reference" ? "Cross-reference" : "A topical study"}</p><h2 className="mt-2 text-4xl text-[var(--color-hero,#006BB3)] md:text-6xl" style={{fontFamily: "var(--font-display, 'Playfair Display', serif)"}}>{selected.title}</h2>{selected.definition && <p className="mt-3 text-lg italic text-[var(--color-text-muted,#2E5A7A)]">{selected.definition}</p>}</div><div className="kg-no-print flex gap-2"><button onClick={() => toggleStudy(selected.id)} aria-pressed={study.includes(selected.id)} className={`kg-focus kg-sans flex items-center gap-2 rounded border border-[var(--color-border-control,#5B9BC4)] px-3 py-2 text-sm font-bold ${study.includes(selected.id) ? "bg-[var(--color-selected-warm,#FFEADB)] text-[var(--color-text,#003A66)] shadow-[inset_0_0_0_1px_var(--color-action-warm,#C45100)]" : "bg-[var(--color-surface,#FFFDFB)] text-[var(--color-text,#003A66)]"}`}>{study.includes(selected.id) ? <BookmarkCheck size={16}/> : <Bookmark size={16}/>} {study.includes(selected.id) ? "Saved" : "Save study"}</button><button onClick={() => copy("topic")} className="kg-focus kg-sans flex items-center gap-2 rounded border border-[var(--color-border-control,#5B9BC4)] bg-[var(--color-surface,#FFFDFB)] px-3 py-2 text-sm font-bold text-[var(--color-text,#003A66)] hover:bg-[var(--color-surface-soft,#E6F5FF)]"><Copy size={16}/> {copied === "topic" ? "Copied" : "Copy refs"}</button></div></div>
