  ...
  return <div className="min-h-[100dvh] text-[#20394a]" style={{ background: "#f3f0e9", fontFamily: "Georgia, 'Times New Roman', serif" }}>
    ...
    <main className="kg-shell mx-auto grid max-w-[1500px] grid-cols-1 md:grid-cols-[270px_1fr] lg:grid-cols-[290px_1fr_265px]">
      <aside className={`${mobileMenu ? "block" : "hidden"} kg-no-print border-r border-[#b9c4c0] bg-[#e8e9e1] md:block`}><div className="sticky top-[95px] max-h-[calc(100dvh-95px)] overflow-y-auto kg-scroll p-5">
        <a href={base("/knowing-god")} onClick={event => { if (event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return; event.preventDefault(); openStart(); }} className="kg-focus kg-sans mb-6 block text-sm text-[#a4532b] underline underline-offset-2 hover:text-[#123f50]">Return to Knowing God Intro</a>
        <button type="button" onClick={() => window.print()} className="kg-focus kg-sans mb-6 flex w-full items-center justify-center gap-2 rounded border border-[#aebcb8] bg-[#f8f7f2] px-3 py-2 text-sm font-bold text-[#123f50] hover:bg-[#f4d5b5]"><Printer size={16}/> Print</button>
      ...
      {view === "start" ? <section className="kg-reading min-w-0 max-w-[920px] px-5 py-8 md:px-10 md:py-12 lg:px-14"><p className="kg-sans text-[11px] font-bold uppercase tracking-[.22em] text-[#b15d2b]">Introduction</p><h2 className="mt-2 text-4xl text-[#123f50] md:text-6xl">Knowing God</h2><p className="mt-4 text-2xl leading-relaxed italic text-[#61736f] md:text-[28px]">Topical Bible Verses on the Nature and Character of the Almighty</p><p className="kg-sans mt-3 text-sm text-[#61736f]">© 2026 by Zinzendorf Mission</p><div className="mt-8 space-y-6 text-[17px] leading-[1.75]"><p>The complete topical Bible offers {index?.counts.topicCount ?? "hundreds of"} topics and {index?.counts.passageCount.toLocaleString() ?? "thousands of"} curated Scripture passages for study, worship, and prayer.</p><a href={base("/knowing-god/introduction")} className="kg-focus kg-sans inline-flex items-center gap-1 font-bold text-[#a4532b] underline">See introductory articles <ChevronRight size={16}/></a><p>Browse A–Z, search topic titles globally, and search definitions and passages as letters are loaded. Save studies locally, copy references, or print your study.</p><button onClick={() => selectedIndex && openTopic(selectedIndex)} className="kg-focus kg-sans bg-[#123f50] px-5 py-3 text-sm font-bold text-white">Browse the topics</button></div></section> :
      <section className="kg-reading min-w-0 max-w-[920px] px-5 py-8 md:px-10 md:py-12 lg:px-14">{!selectedIndex ? <div role="status" className="kg-sans py-16 text-center">Loading selected topic…</div> : !selected ? <div className="kg-sans py-16 text-center">{payloadError[selectedIndex.letter] ? <div role="alert">{payloadError[selectedIndex.letter]} <button className="underline" onClick={() => loadLetter(selectedIndex.letter).catch(() => undefined)}>Try again</button></div> : <div role="status">Loading {selectedIndex.letter} topics…</div>}</div> : <><div className="kg-no-print mb-6 flex items-center gap-2 kg-sans text-xs"><BookOpen size={15}/>Topic study <ChevronRight size={13}/>{selected.title}</div><div className="mb-8 flex flex-col gap-4 border-b pb-7 sm:flex-row sm:justify-between"><div><p className="kg-sans text-[11px] font-bold uppercase tracking-[.22em] text-[#b15d2b]">{selected.recordType === "cross-reference" ? "Cross-reference" : "A topical study"}</p><h2 className="mt-2 text-4xl text-[#123f50] md:text-6xl">{selected.title}</h2>{selected.definition && <p className="mt-3 text-lg italic text-[#61736f]">{selected.definition}</p>}</div><div className="kg-no-print flex gap-2"><button onClick={() => toggleStudy(selected.id)} className="kg-focus kg-sans border px-3 py-2 text-xs">{study.includes(selected.id) ? <BookmarkCheck size={15}/> : <Bookmark size={15}/>} {study.includes(selected.id) ? "Saved" : "Save study"}</button><button onClick={() => copy("topic")} className="kg-focus kg-sans border px-3 py-2 text-xs"><Copy size={15}/> {copied === "topic" ? "Copied" : "Copy refs"}</button></div></div>
        ...
        <div className="mb-5 flex justify-between"><h3 className="text-2xl text-[#123f50]">Scripture passages <span className="kg-sans text-xs">{visiblePassages.length} results</span></h3><button onClick={() => copy("passages")} className="kg-focus kg-no-print kg-sans text-xs"><Clipboard size={14}/> {copied === "passages" ? "Copied" : "Copy passages"}</button></div>{netLoading && translation === "NET" ? <div role="status" className="kg-sans py-10 text-center">Loading NET Bible text…</div> : <div>{visiblePassages.map((p, i) => { const net = netCache.get(p.reference); return <article key={`${p.reference}-${i}`} className="border-b py-4"><p className="kg-sans text-sm font-bold text-[#285c68]">{p.reference} {translation === "NET" && <span className="text-[10px]">{net ? "NET" : "Fallback KJV"}</span>}</p><p className="mt-2 text-[17px] leading-[1.75]">{translation === "NET" ? net || p.text : p.text}</p></article>; })}</div>}{visiblePassages.length === 0 && <div className="border border-dashed py-10 text-center">This cross-reference or filter has no passages to display.</div>}{selected.additionalScripture.length > 0 && <div className="mt-10 border-t pt-6"><p className="kg-sans text-[10px] font-bold uppercase">Additional Scripture</p>{selected.additionalScripture.map((section, sectionIndex) => <p className="mt-2" key={`${selected.id}-additional-${sectionIndex}`}>{section.links.map((reference, index, all) => { const unchanged = reference.queries.length === 1 && normalized(reference.sourceLabel.replace(/\.$/, "")) === normalized(reference.queries[0]); return <span key={`${reference.sourceLabel}-${index}`}>{unchanged ? <a className="kg-focus text-[#a4532b] underline" href={scriptureHref(reference.queries[0])} target="_blank" rel="noopener noreferrer">{reference.sourceLabel}</a> : <span>{reference.sourceLabel} <span className="kg-sans text-xs text-[#61736f]">({reference.queries.map((query, queryIndex) => <span key={query}><a className="kg-focus text-[#a4532b] underline" href={scriptureHref(query)} target="_blank" rel="noopener noreferrer">{query}</a>{queryIndex < reference.queries.length - 1 ? ", " : ""}</span>)})</span></span>}{index < all.length - 1 ? "; " : ""}</span>; })}</p>)}</div>}</>}</section>}
      <aside className="kg-no-print border-l bg-[#ecebe4] p-6 lg:min-h-[calc(100dvh-95px)]"><div className="mb-8"><div className="mb-3 flex gap-2 text-[#b15d2b]"><Bookmark size={16}/><h3 className="kg-sans text-xs font-bold uppercase">My study list</h3></div>{study.length ? study.map(id => { const item = topicById(id); return item ? <div className="flex border-b" key={id}><button className="kg-focus flex-1 py-2 text-left text-sm" onClick={() => openTopic(item)}>{item.title}</button><button className="kg-focus" onClick={() => toggleStudy(id)} aria-label={`Remove ${item.title} from study list`}><X size={15}/></button></div> : null; }) : <p className="kg-sans text-sm text-[#72817e]">Save topics here as you trace a theme through Scripture.</p>}</div>{view === "topic" && selected && <div className="border-t pt-6"><p className="kg-sans text-[10px] font-bold uppercase">See also</p><div className="mt-3 space-y-3">{selected.seeAlso.map(related => <div key={related.sourceLabel}><p className="text-sm">{related.sourceLabel}</p><div className="mt-1 flex flex-wrap gap-1">{related.targetIds.map(id => { const target = id === "*" ? undefined : topicById(id); return <button key={id} onClick={() => id === "*" ? openStart() : target && openTopic(target)} className="kg-focus border border-[#aebcb8] px-2 py-1 kg-sans text-[11px] font-semibold text-[#285c68]">{id === "*" ? "Browse all topics" : target?.title}</button>; })}</div></div>)}</div></div>}<div className="mt-10 border-t pt-6"><p className="kg-sans text-[10px] font-bold uppercase">About Knowing God</p><p className="mt-2 text-sm text-[#687974]">This digital topical Bible includes the complete generated corpus: {index?.counts.topicCount ?? 773} topics and {(index?.counts.passageCount ?? 13506).toLocaleString()} passages.</p></div></aside>
