...
export function ConcordancePrototype() {
  ...
  const [study, setStudy] = useState<string[]>(() => { try { return JSON.parse(localStorage.getItem("knowing-god-study") || "[]"); } catch { return []; } });
  const [copied, setCopied] = useState("");
  const selected = topics.find(t => t.title === selectedTitle) || topics[0];

  useEffect(() => { localStorage.setItem("knowing-god-study", JSON.stringify(study)); }, [study]);
      ...
      <header className="kg-no-print sticky top-0 z-30 border-b border-[#274c5b]/20 bg-[#123f50] text-[#f8f5ed]">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-3 md:px-8">
          <div className="flex items-center gap-3">
            ...
            <div><p className="kg-sans text-[10px] font-semibold uppercase tracking-[.25em] text-[#bdd0cc]">JO EQUIP · Zinzendorf Mission</p><h1 className="text-xl tracking-tight md:text-2xl">Knowing God <span className="text-[#e4b16d]">/</span> Concordance</h1></div>
          </div>
      ...
      <main className="kg-shell mx-auto grid max-w-[1500px] grid-cols-1 md:grid-cols-[270px_1fr] lg:grid-cols-[290px_1fr_265px]">
        <aside className={`${mobileMenu ? "block" : "hidden"} kg-no-print border-r border-[#b9c4c0] bg-[#e8e9e1] md:block`}>
          <div className="sticky top-[95px] max-h-[calc(100dvh-95px)] overflow-y-auto kg-scroll p-5">
            ...
            <div className="relative mb-4"><Search className="absolute left-3 top-3 text-[#6c7f7d]" size={16}/><input id="topic-search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search the concordance" className="kg-focus w-full border border-[#aebcb8] bg-[#f8f7f2] py-2.5 pl-9 pr-3 text-sm text-[#20394a] placeholder:text-[#80908d]"/></div>
