...
import { buildAdventurePublicationPages } from "./adventurePublicationPages.js";
import {
  applyBookPublicationPages,
  BOOK_PUBLICATION_PAGES_CSS,
  loadBookPublicationPages,
  type BookPublicationPages,
...
function linkifyOutsideAnchors(html: string): string {
  ...
  return parts
    .map((part, i) => {
      ...
      let out = part;
      for (const [re, fn] of bare) {
        // Preserve anchors generated by earlier patterns, not only source anchors.
        out = out.split(/(<a [\s\S]*?<\/a>)/)
          .map((segment, index) => index % 2 ? segment : segment.replace(re, fn))
          .join("");
      }
      return out;
    })
...
function cssFor(book: BookConfig): string {
  ...
  if (["walking", "identity", "majesty"].includes(book.key)) {
    return base + BOOK_PUBLICATION_PAGES_CSS +
      (book.key === "majesty" && book.source === "docx" ? MAJESTY_DOCX_CSS : "");
  }
...
async function buildBook(browser: Browser, book: BookConfig, resourceHtml: string, publicationPages: BookPublicationPages): Promise<void> {
  console.log(`\n=== ${book.title} ===`);
  if (book.source === "approved-pdf") {
    if (book.key === "adventure") {
      await buildAdventurePublicationPages({
        approvedPdfPath: book.docx,
        outputPdfPath: book.out,
        resourcesDocxPath: ADDITIONAL_RESOURCES_DOCX,
      }, browser);
      return;
    }
...
async function main(): Promise<void> {
  ...
  const publicationPages = await loadBookPublicationPages(ADDITIONAL_RESOURCES_DOCX);
  try {
    for (const book of selected) await buildBook(browser, book, resourceHtml, publicationPages);
  } finally {
    await browser.close();
  }
}
